...
#YW note: defined 1 class: X_BasicInfo
# 2020/08/02 github update: added one function: collect_basic_info_one_sample
...
####
    ...
    #         frcov = record[3]
    #         f_cov = (flcov + frcov) / 2
    #         # YW 2020/06/27 this if statement makes the coverage estimation higher than it should be for ancient DNA especially
...
####
    ...
    #randomly select some sites, and calc the coverage, insert size, and read length
    def collect_basic_info_samples(self, sf_bam_list, sf_ref, search_win):
        ...
        l_is = []  # insert size and std deviation for each bam
        ...
        with open(sf_bam_list) as fin_bams:
            for line in fin_bams:
                ...
                for record in l_tmp_info:
                    ...
                    f_cov=(flcov+frcov)/2
                    # YW 2020/07/19 only doing it for the testing samples
                    # if f_cov >= global_values.MIN_COV_RANDOM_SITE:  # for WES, skip those intron regions
                    l_cov.append(f_cov)
    ...
    def collect_basic_info_of_sites2(self, l_sites, sf_bam, search_win, focal_win):
        ...
        for (chrm, pos) in l_sites:
            # l_records.append(((chrm, pos, search_win, focal_win), sf_bam, self.working_folder)) # YW 2020/07/19 commented out
            # YW 2020/07/19 added this for benchmarking:
            if chrm == "chr1":
...
####
        ...
        # first get the dominant read length (mode)
        domint_rlth = 0
        cnt_tmp = 0
...
####
    ...
    #calc the mean, std deviation of the insert size
