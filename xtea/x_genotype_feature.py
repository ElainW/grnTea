...
# YW 2020/07/16 note: defined python class: XGntpFeatures and XGenotyper
# YW 2020/08/02: originally x_genotype.py with additional functions from x_polyA and other minor changes
...
def unwrap_self_collect_clip_disc_features(arg, **kwarg):
    ...
    return XGenotyper.collect_features_one_chr(*arg, **kwarg)
...
class XGenotyper():
    ...
    def call_genotype(self, sf_bam_list, sf_candidate_list, extnd, sf_out):
        ...
        for sf_bam in l_bams:
            ...
            l_chrm_records = [] # YW 2020/07/24 change from chr-pos pair to chr
            d_chrm_records = {}
            with open(sf_candidate_list) as fin_list:
                for line in fin_list:
                    ...
                    # YW 2020/07/24 to reduce the number of genotype files generated at one time, make a chr_specific genotype file
            ...
            for chrm in d_chrm_records:
                l_chrm_records.append(((chrm, d_chrm_records[chrm], extnd), sf_bam, self.working_folder))
            ...
            with open(sf_out_features, "w") as fout_all:
                for rcd in l_chrm_records:
                    chrm=rcd[0][0]
                    # ins_pos=rcd[0][1]
                    # s_pos_info = "{0}_{1}".format(chrm, ins_pos) # need to change this too, to chr based
                    # sf_gntp_features = self.working_folder + s_pos_info + global_values.GNTP_FEATURE_SUFFIX
                    sf_gntp_features = self.working_folder + chrm + global_values.GNTP_FEATURE_SUFFIX
                ...
                for ins_chrm in m_info:
                    for ins_pos in m_info[ins_chrm]:
                        if (ins_chrm in m_merged) and (ins_pos in m_merged[ins_chrm]):
                            #merge the information, YW 2020/07/16 the keys of dict m_info were wrong, fixed it
...
####
    ...
    def collect_features_one_site(self, record):
        ...
        n_l_af_clip=0# num of left clipped reads for checking allele freqency
        n_r_af_clip=0# num of right clipped reads for checking allele frequency
        n_full_map = 0
...
####
        ...
        samfile.close()
#
    # YW 2020/07/24 adapted from collect_features_one_site, same output format
    ####collect clip and discordant features of given site
    ####for each candidate site:
    #Collect: 1. # of left clipped reads; 2. # of right clipped reads;
    ####3. # of fully mapped reads bypass the site; 4. # of reads contain small indels
    ...
    #return info:
    ####chrm pos n_af_clip n_full_map n_l_raw_clip n_r_raw_clip n_disc_pairs n_concd_pairs n_disc_large_indel s_clip_lens        
    def collect_features_one_chr(self, record):
        chr = record[0][0]  ##this is the chrm style in candidate list
        l_ins_pos = record[0][1]
        extnd = record[0][2]
        ...
        sf_gntp_features = working_folder + chr + global_values.GNTP_FEATURE_SUFFIX
        f_gntp_fetures = open(sf_gntp_features, "w")
        
        for ins_pos in l_ins_pos:
            start_pos = ins_pos - extnd
            if start_pos <= 0:
                start_pos = 1
            end_pos = ins_pos + extnd
...
####
        ...
        # YW 2020/07/24 added this to remove empty files
        if os.path.isfile(sf_gntp_features):
            if os.path.getsize(sf_gntp_features)==0:
                os.remove(sf_gntp_features)
####
...
####This is drectly copies from x_clip_disc_filter.py
    ...
    # here require two reads be at the two sides of the insertion
...
#### YW 2020/08/05 github update has problems (to email Simon), n_polyA and n_disc_chrms aren't extracted in the following steps
