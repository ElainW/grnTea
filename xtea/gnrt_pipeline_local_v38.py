...
# YW 2020/07/24 customize memory allocation for each TE
def gnrt_script_head(spartition, ncores, stime, smemory, s_id, rep_type):
    s_head = "#!/bin/bash\n\n"
    s_head += "#SBATCH -c {0}\n".format(ncores)
    s_head += "#SBATCH -t {0}\n".format(stime)
    ...
    s_head += "#SBATCH -p {0}\n".format(spartition)
    s_head += "#SBATCH -o {0}_{1}_%j.out\n".format(s_id, rep_type)
...
#### YW 2020/08/04 modified github update: added rep_type to this new function
def gnrt_script_head_lsf(spartition, ncores, stime, smemory, s_id, rep_type):
    s_head = "#!/bin/bash\n\n"
    s_head += "#BSUB -n {0}\n".format(ncores)
    ...
    s_head += "#BSUB -R \"rusage[mem={0}G]\"\n".format(int(memory)/int(ncores)) #this is memory per core
    return s_head
...
def gnrt_calling_command(iclip_c, iclip_rp, idisc_c, iflt_clip, iflt_disc, ncores, iflk_len, min_tei_len, iflag,
                         b_mosaic, b_user_par, b_force, b_tumor, b_resume, f_purity, i_rep_type, s_cfolder, b_SVA=False, b_L1=False):
    s_user = ""
    if b_user_par == True:
        s_user = "--user"
    s_clean = ""
    ...
    sclip_step = "python3 ${{XTEA_PATH}}\"x_TEA_main.py\" -C -i ${{BAM_LIST}} --lc {0} --rc {1} --cr {2}  " \
    ...
    sdisc_step = "python3 ${{XTEA_PATH}}\"x_TEA_main.py\"  -D -i ${{PREFIX}}\"candidate_list_from_clip.txt\" --nd {0} " \
    ...
    if b_mosaic is True and b_SVA is True:
        ...
        sdisc_step = "python3 ${{XTEA_PATH}}\"x_TEA_main.py\"  -D --sva --somatic -i ${{PREFIX}}\"candidate_list_from_clip.txt\" --nd {0} " \
                     "--ref ${{REF}} -a ${{ANNOTATION}} -b ${{BAM_LIST}} -p ${{TMP}} " \
                     "-o ${{PREFIX}}\"candidate_list_from_disc.txt\" -n {1} {2}\n".format(idisc_c, ncores, s_user)
    sbarcode_step = "python3 ${{XTEA_PATH}}\"x_TEA_main.py\" -B -i ${{PREFIX}}\"candidate_list_from_disc.txt\" --nb 400 " \
                    "--ref ${{REF}} -a ${{ANNOTATION}} -b ${{BAM1}} -d ${{BARCODE_BAM}} -p ${{TMP}} " \
                    "-o ${{PREFIX}}\"candidate_list_barcode.txt\" -n {0} {1}\n".format(ncores, s_user)
    sfilter_10x = "python3 ${{XTEA_PATH}}\"x_TEA_main.py\" -N --cr {0} --nd {1} -b ${{BAM_LIST}} -p ${{TMP_CNS}} " \
    ...
    s_filter = "python3 ${{XTEA_PATH}}\"x_TEA_main.py\" -N --cr {0} --nd {1} -b ${{BAM_LIST}} -p ${{TMP_CNS}} " \
...
def gnrt_lib_config(l_rep_type, sf_folder_rep, sf_ref, sf_gene, sf_black_list, sf_folder_xtea, sf_config_prefix):
    ...
    for s_rep_type in l_rep_type:
        if s_rep_type=="L1":# for L1
            ...
            sf_anno = "ANNOTATION " + sf_folder_rep + "L1/hg38/hg38_L1_larger_500_with_all_L1HS.out\n"
            sf_anno1 = "ANNOTATION1 " + sf_folder_rep + "L1/hg38/hg38_L1.fa.out\n"
            ...
            sf_copy_with_flank = "L1_COPY_WITH_FLANK " + sf_folder_rep + "L1/hg38/hg38_L1_copies_larger_5K_with_flank.fa\n"
