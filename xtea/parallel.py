		...
		with open(bash_script, 'w') as fout:
			...
			for scr in scr_list:
				...
				with open(sbatch_script, 'w') as sub_fout:
					...
					sheader += f"#SBATCH -J {global_values.SAMPLE_ID}_{self.idx_bam}_{rep_type}_{option}_cns_realn\n"
		...
		with open(bash_script, 'w') as fout:
			...
			for i, scr in enumerate(scr_list):
				...
				with open(sbatch_script, 'w') as sub_fout:
					...
					sheader += "#SBATCH -N 1\n"
					sheader += f"#SBATCH -c {self.n_jobs}\n"
					sheader += f"#SBATCH -J {global_values.SAMPLE_ID}_{idx}_feat_extraction\n"
