                        ...
                        for line in fin_tmp:
                            ...
                            if pos not in m_sites_chrm:
                                ...
                                for value in fields[2:]:
                                    ...
                                    # YW note 2020/06/30: actually all the positions only appear once (due to usage of python dict in call_TEI_candidate_sites_from_clip_reads_v2), so unnecessary to sum here
...
####
    ...
    def filter_candidate_sites_by_discordant_pairs_multi_alignmts(self, m_sites, iext, i_is, f_dev, cutoff,
        ...
        # YW 2020/07/05 clarified the comment below
        # merge the output by summing up all the discordant reads in different bam files of the same sample
        # output the instances with left disc or right disc reaching the threshold in a single file
        m_merged_sites = {}
        for i in range(cnt):
...
####
        ...
        clip_info.cnt_clip_part_aligned_to_rep(sf_algnmt)  ##require at least 3/4 (YW changed from half from previous comment) of the seq is mapped !!!!
